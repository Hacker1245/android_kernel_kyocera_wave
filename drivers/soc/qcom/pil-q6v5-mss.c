...
#define SECURITY_ERROR    "security check Error!"
...
static void log_modem_sfr(void)
{
	...
	
	{
		...
		if (reason_len > sizeof(SECURITY_ERROR))
		{
			/* printk("reason:%s\n", &reason[reason_len - sizeof(SECURITY_ERROR) + 1]); */
			if (strncmp(&reason[reason_len - sizeof(SECURITY_ERROR) + 1], SECURITY_ERROR, sizeof(SECURITY_ERROR)) == 0)
			{
				set_auth_error_mode();
				machine_restart("oem-1");
			}
		}
	}

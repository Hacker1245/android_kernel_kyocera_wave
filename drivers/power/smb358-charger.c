...
#define CHG_CTRL_SWITCH_FREQ_BIT	BIT(7)
#define CHG_CTRL_SWITCH_FREQ_MASK	BIT(7)
...
static int smb358_hw_init(struct smb358_charger *chip)
{
	...
	reg = LIMIT_CELL_HW_COLD_BIT | LIMIT_CELL_HW_HOT_BIT;
	...
	if (OEM_get_board() >= OEM_BOARD4_TYPE) {
		/* 
		* Workaround
		*/
		rc = __smb358_path_suspend(chip, true);
		if (!rc) {
			rc = smb358_masked_write(chip, THERM_A_CTRL_REG, CHG_CTRL_SWITCH_FREQ_MASK, CHG_CTRL_SWITCH_FREQ_BIT);
			if (rc) 
				dev_err(chip->dev, "Couldn't change  rc=%d\n", rc);
...
static int smb_parse_dt(struct smb358_charger *chip)
{
	...
	if(oem_fact_get_option_bit(OEM_FACT_OPTION_ITEM_01, 6) || OEM_get_board() == OEM_BOARD5_TYPE) {
...
{
	struct smb358_charger *chip = container_of(work,
				struct smb358_charger, oem_off_chg_check_work.work);

...
#define DETERIORATION_THRESH_BETTER				80
#define DETERIORATION_THRESH_GOOD				60
#define DETERIORATION_THRESH_GOOD_TO_BETTER		90
...
{
	int vbatt_now_uv = oem_bms_get_batt_prop(POWER_SUPPLY_PROP_VOLTAGE_NOW);
...
static void oem_bms_update_fcc_learning(void)
{
	...
	if ((estimate_mah >= oem_param_chg_cycle.learning_mah) &&
		(estimate_mah <= (oem_param_chg_cycle.learning_mah * 12 / 10))) {
...
static void oem_bms_fcc_learning_monitor_work(struct work_struct *work)
{
	...
	if (batt_temp >= STOP_BATT_TEMP_HI) {
		pr_info("STOP_BATT_TEMP_HI:%d\n", batt_temp);
		learning_status = LEARNING_STATUS_MONITOR_STOP;
		wake_unlock(&fcc_learning_wake_lock);
		return;
	}

	oem_chg_vadc_read(P_MUX2_1_1, &result);
...
void oem_bms_start_fcc_learning(int soc_final)
{
	...
	if ((batt_temp >= START_BATT_TEMP_LO) &&
		(batt_temp <= START_BATT_TEMP_HI) &&
		(soc_final <= SOC_START)) {
		...
		learning_status = LEARNING_STATUS_MONITOR_STOP;
		pr_info("do not monitor batt_temp:%d soc_final:%d\n",
			batt_temp, soc_final);
	}
}

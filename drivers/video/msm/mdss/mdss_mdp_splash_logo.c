...
#endif
extern int oem_chg_is_recoverymode(void);
extern int oem_chg_is_fotamode(void);
...
#ifdef CONFIG_DISP_EXT_KSPLASH
	kernel_disp_raw_bitmap_SR(0,0,540,960, splash_bgr888_image, sinfo->splash_buffer, 0);
#else
...
int mdss_mdp_splash_init(struct msm_fb_data_type *mfd)
{
	...
	if (oem_chg_is_off_charge()) {
		rc = -EINVAL;
		goto end;
	}
	if(oem_chg_is_recoverymode()){
		rc = -EINVAL;
		goto end;
	}
	
	if(oem_chg_is_fotamode()){
		rc = -EINVAL;
		goto end;
	}

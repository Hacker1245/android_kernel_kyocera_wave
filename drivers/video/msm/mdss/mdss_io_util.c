...
int msm_dss_enable_vreg(struct dss_vreg *in_vreg, int num_vreg, int enable)
{
	...
	if (enable) {
		...
		for (i = num_vreg-1; i >= 0; i--) {
			if (!strcmp(in_vreg[i].vreg_name, "camvio")) {
				if (regulator_is_enabled(in_vreg[1].vreg)) {
					if (in_vreg[1].pre_off_sleep)
						usleep(in_vreg[1].pre_off_sleep * 1000);
					regulator_set_optimum_mode(in_vreg[1].vreg,
						in_vreg[1].disable_load);
					regulator_disable(in_vreg[1].vreg);
					if (in_vreg[1].post_off_sleep)
						usleep(in_vreg[1].post_off_sleep * 1000);
				}
				if (regulator_is_enabled(in_vreg[2].vreg)) {
					if (in_vreg[2].pre_off_sleep)
						usleep(in_vreg[2].pre_off_sleep * 1000);
					regulator_set_optimum_mode(in_vreg[2].vreg,
						in_vreg[2].disable_load);
					regulator_disable(in_vreg[2].vreg);
					if (in_vreg[2].post_off_sleep)
						usleep(in_vreg[2].post_off_sleep * 1000);
				}
			} else if (!strcmp(in_vreg[i].vreg_name, "vdd")) {
				;
			} else {

...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (!disp_ext_otp_check(ctrl)) {
		if (ctrl->no_otp_cmds.cmd_cnt)
			mdss_dsi_panel_cmds_send(ctrl, &ctrl->no_otp_cmds);
	}

	if (ctrl->on2_cmds.cmd_cnt)
		mdss_dsi_panel_cmds_send(ctrl, &ctrl->on2_cmds);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->no_otp_cmds,
		"qcom,mdss-dsi-no-otp-command", "qcom,mdss-dsi-no-otp-command-state");

	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->on2_cmds,
		"qcom,mdss-dsi-on2-command", "qcom,mdss-dsi-on2-command-state");

...
static ssize_t mdss_fb_set_invert_color(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = fbi->par;
	...
	rc = kstrtoint(buf, 10, &invert_color);
	if (rc) {
		pr_err("kstrtoint failed. rc=%d\n", rc);
		return rc;
	}

	pr_debug("invert_color = %d\n", invert_color);
	lock_fb_info(mfd->fbi);
	disp_ext_util_set_invert_color(mfd, invert_color);
	unlock_fb_info(mfd->fbi);
...
#endif
static DEVICE_ATTR(invert_color, S_IWUSR, NULL, mdss_fb_set_invert_color);
						...
						unsigned long *argp)
{
	...
	if (disp_ext_diag_event_flag_check() & (IMG_ERR_CHECK | MIPI_ERR_CHECK)) {
		...
		MIPI_OUTP((ctrl_pdata->ctrl_base) + 0x098, 0x00);
		wmb();
	}
#endif

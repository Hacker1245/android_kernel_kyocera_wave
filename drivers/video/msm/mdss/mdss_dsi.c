...
static int mdss_dsi_pinctrl_init(struct platform_device *pdev)
{
	...
	ctrl_pdata->pin_res.gpio_state_det_active1
		= pinctrl_lookup_state(ctrl_pdata->pin_res.pinctrl,
				MDSS_PINCTRL_STATE_DET_ACTIVE1);
	if (IS_ERR_OR_NULL(ctrl_pdata->pin_res.gpio_state_det_active1))
		pr_warn("%s: can not get det_active1 pinstate\n", __func__);

	ctrl_pdata->pin_res.gpio_state_det_active2
		= pinctrl_lookup_state(ctrl_pdata->pin_res.pinctrl,
				MDSS_PINCTRL_STATE_DET_ACTIVE2);
	if (IS_ERR_OR_NULL(ctrl_pdata->pin_res.gpio_state_det_active2))
		pr_warn("%s: can not get det_active2 pinstate\n", __func__);

	ctrl_pdata->pin_res.gpio_state_id0_active1
		= pinctrl_lookup_state(ctrl_pdata->pin_res.pinctrl,
				MDSS_PINCTRL_STATE_ID0_ACTIVE1);
	if (IS_ERR_OR_NULL(ctrl_pdata->pin_res.gpio_state_id0_active1))
		pr_warn("%s: can not get id0_active1 pinstate\n", __func__);

	ctrl_pdata->pin_res.gpio_state_id0_active2
		= pinctrl_lookup_state(ctrl_pdata->pin_res.pinctrl,
				MDSS_PINCTRL_STATE_ID0_ACTIVE2);
	if (IS_ERR_OR_NULL(ctrl_pdata->pin_res.gpio_state_id0_active2))
		pr_warn("%s: can not get id0_active2 pinstate\n", __func__);

	ctrl_pdata->pin_res.gpio_state_id1_active1
		= pinctrl_lookup_state(ctrl_pdata->pin_res.pinctrl,
				MDSS_PINCTRL_STATE_ID1_ACTIVE1);
	if (IS_ERR_OR_NULL(ctrl_pdata->pin_res.gpio_state_id1_active1))
		pr_warn("%s: can not get id1_active1 pinstate\n", __func__);

	ctrl_pdata->pin_res.gpio_state_id1_active2
		= pinctrl_lookup_state(ctrl_pdata->pin_res.pinctrl,
				MDSS_PINCTRL_STATE_ID1_ACTIVE2);
	if (IS_ERR_OR_NULL(ctrl_pdata->pin_res.gpio_state_id1_active2))
		pr_warn("%s: can not get id1_active2 pinstate\n", __func__);
...
#ifndef CONFIG_DISP_EXT_PANEL_ID
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	dsi_pan_node = disp_ext_find_panel_of_node(pdev);
#else
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->lcd_det_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "qcom,platform-lcd_det-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->lcd_det_gpio))

...
static ssize_t camera_flash_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	int ret;
	static struct regulator *vreg_ldo15 = NULL;

	fctrl = oem_bd7710_get_fctrl();
	if (!fctrl) {
		pr_err("fctrl NULL\n");
		return -EINVAL;
	}
	...
	if (value < MSM_CAMERA_LED_OFF || MSM_CAMERA_LED_HIGH < value) {
		return -EINVAL;
	}

	if(value != MSM_CAMERA_LED_OFF) {
		if(!vreg_ldo15) {
			vreg_ldo15 = regulator_get(NULL, "8916_l15");
			if (IS_ERR(vreg_ldo15)) {
				pr_err("%s: vreg get l15-reg failed (%ld)\n",
					__func__, PTR_ERR(vreg_ldo15));
				return PTR_ERR(vreg_ldo15);
			}
			ret = regulator_set_voltage(vreg_ldo15, 1800000, 1800000);
			if (ret < 0)
				pr_err("%s: unable to set_voltage ldo15 voltage\n", __func__);

			ret = regulator_set_optimum_mode(vreg_ldo15, 10000);
			if (ret)
				pr_err("%s: unable to set_optimum_mode ldo15 voltage\n", __func__);

			ret = regulator_enable(vreg_ldo15);
			if (ret)
				pr_err("%s: unable to enable ldo15 voltage\n", __func__);
		}
		cfg.cfgtype = MSM_CAMERA_LED_INIT;
		fctrl->func_tbl->flash_led_config(fctrl, (void *)&cfg);
		...
		if(vreg_ldo15) {
			...
			fctrl->func_tbl->flash_led_config(fctrl, (void *)&cfg);
			ret = regulator_disable(vreg_ldo15);
			if (ret)
				pr_err("%s: unable to enable ldo15 voltage\n", __func__);
			regulator_set_optimum_mode(vreg_ldo15, 0);
			regulator_set_voltage(vreg_ldo15, 0, 1800000);
			regulator_put(vreg_ldo15);
			vreg_ldo15 = NULL;
		}
	}
	return size;

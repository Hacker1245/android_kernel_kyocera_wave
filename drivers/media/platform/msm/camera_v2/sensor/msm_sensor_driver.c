...
static gpio_info_t gpio_cam_id[2] = {
	{
		0,
		"cam_main_id",
	},
	...
	struct device_node *of_node)
{
	...
	gpio_rc = of_property_read_u32(of_node, "qcom,gpio-cam-main-id", &val);
	if (gpio_rc != -EINVAL) {
		if (!(gpio_rc < 0) && !(val >= gpio_array_size)) {
			gpio_cam_id[0].no = gpio_array[val];
			CDBG("%s gpio-cam-main-id %d\n", __func__,
				gpio_cam_id[0].no);
		}
	}
	gpio_rc = of_property_read_u32(of_node, "qcom,gpio-cam-sub-id", &val);
	if (gpio_rc != -EINVAL) {
		if (!(gpio_rc < 0) && !(val >= gpio_array_size)) {
			gpio_cam_id[1].no = gpio_array[val];
			CDBG("%s gpio-cam-sub-id %d\n", __func__,
				gpio_cam_id[1].no);
		}
	}
...
uint32_t msm_sensor_driver_get_cam_gpio(int32_t index)
{
	uint32_t rc;

	gpio_request(gpio_cam_id[index].no, gpio_cam_id[index].name);
	rc = gpio_get_value_cansleep(gpio_cam_id[index].no);
	gpio_free(gpio_cam_id[index].no);

	CDBG("%s gpio[%d]:value[%d]\n", __func__, gpio_cam_id[index].no, rc);
	return rc;
}

...
static int cyttsp5_change_state(struct device *dev);
		...
		u8 test_id, u8 *status, u16 *actual_read_len, u8 *data)
{
	...
	repeat = test_id == CY_ST_ID_CUSTOM_OPENS;
...
static int _cyttsp5_request_set_state_loading(struct device *dev, bool set)
{
	...
	if(set){
		cd->oem_state |= OEM_STATE_LOADING_BIT;
	}
	else{
		cd->oem_state &= ~OEM_STATE_LOADING_BIT;
	}
	rc = cyttsp5_change_state(cd->dev);
...
#ifdef CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH
	mutex_lock(&cd->state_lock);
	cd->oem_state |= OEM_STATE_STARTUP_BIT;
	cyttsp5_change_state(cd->dev);
	mutex_unlock(&cd->state_lock);
#endif /* CONFIG_TOUCHSCREEN_KC_SENSOR_SWITCH */
...
static int cyttsp5_pinctrl_select(struct cyttsp5_core_data *core_data, bool on)
{
...
static int cyttsp5_change_state(struct device *dev)
{
	...
	dev_dbg(dev, "%s: mode change 0x%04x ==> 0x%04x\n",__func__,cd->old_state,cd->oem_state);
	if(cd->old_state == cd->oem_state) {
		goto done;
	}
	if(0 == cd->old_state && 0 < cd->oem_state) {
	...
	else 	if(0 < cd->old_state && 0 == cd->oem_state) {
...
static int cyttsp5_eco_and_suspend(struct cyttsp5_core_data *cd)
{
	...
	msleep(30);
	rc = regulator_set_optimum_mode(cd->sensor_avdd, 100);
	if(rc < 0){
...
static int cyttsp5_normal_and_resume(struct cyttsp5_core_data *cd)
{
	...
	rc = regulator_set_optimum_mode(cd->sensor_avdd, 30000);
	if(rc < 0){
		pr_err("%s: vdd: set_optimum Error. err = %d\n", __func__, rc);
		return -EAGAIN;
	}
...
static int cyttsp5_sensor_switch_ioctl(struct kc_ts_data *ts, int value)
{
	...
	if (value) {
		dev_info(cd->dev, "%s: KC Force Enable ON\n", __func__);
		mutex_lock(&cd->state_lock);
		cd->oem_state |= OEM_STATE_SWITCH_BIT;
		cyttsp5_change_state(cd->dev);
		mutex_unlock(&cd->state_lock);
	}
	else {
		dev_info(cd->dev, "%s: KC Force Enable OFF\n", __func__);
		mutex_lock(&cd->state_lock);
		cd->oem_state &= ~OEM_STATE_SWITCH_BIT;
		cyttsp5_change_state(cd->dev);
		mutex_unlock(&cd->state_lock);
		...
		u16 irq, size_t xfer_buf_size)
{
    ...
    cd->sensor_vdd = regulator_get(dev, "vcc_i2c");
	if(IS_ERR(cd->sensor_vdd)) {
		pr_err("%s: vcc_i2c: regulator_get fail.\n", __func__);
		err = PTR_ERR(cd->sensor_vdd);
		goto error_regulator;
	}
	if(regulator_is_enabled(cd->sensor_vdd))
		pr_info("%s: vcc_i2c: regulator is enbale.\n", __func__);
	else {
		pr_err("%s: vcc_i2c: regulator is disable.\n", __func__);
		err = regulator_enable(cd->sensor_vdd);
		if(err) {
			pr_err("%s: vcc_i2c: regulator_enable fail. err=%d\n", __func__, err);
			goto error_regulator;
		}
		usleep_range(1000,1000);
	}

	cd->sensor_avdd = regulator_get(dev, "vdd");
	if(IS_ERR(cd->sensor_avdd)){
		pr_err("%s: vdd: regulator_get fail.\n", __func__);
		goto error_regulator;
	} else {
		err = regulator_set_voltage(cd->sensor_avdd, 3000000, 3000000);
		if(err < 0){
			pr_err("%s: vdd: set_voltage Error. err = %d\n", __func__, err);
			goto error_regulator;
		}
		err = regulator_set_optimum_mode(cd->sensor_avdd, 30000);
		if(err < 0){
			pr_err("%s: vdd: set_optimum Error. err = %d\n", __func__, err);
			goto error_regulator;
		} else
			pr_info("%s: vdd: set_optimum = 0x%02d.\n", __func__, err);
		err = regulator_enable(cd->sensor_avdd);
		if(err){

...
#define DEFAULT_OPENS_IDAC_MUTUAL	10
#define DEFAULT_OPENS_ATTEN_MUTUAL	10
...
struct cyttsp5_device_access_data {
	...
	u32 opens_idac_mutual;
	u32 opens_atten_mutual;
#ifdef TTHE_TUNER_SUPPORT
 ...
 */
static int _cyttsp5_get_parameter_cmd(struct device *dev,
		u8 param_id, u32 *buf)
{
	int rc;
	rc = cmd->nonhid_cmd->get_param(dev, 0, param_id,
			buf);
	return rc;
}
 ...
 */
static int _cyttsp5_set_parameter_cmd(struct device *dev,
		u8 param_id, u32 value)
{
	int rc;
	rc = cmd->nonhid_cmd->set_param(dev, 0, param_id,
			value);
	return rc;
}
		...
		struct cyttsp5_attribute *attr, char *buf)
{
	...
	cd = cyttsp5_get_core_data((char *)CY_DEFAULT_CORE_ID);

	/* Form response buffer */
	memcpy(dad->ic_buf, cd->response_buf, 7);
		...
		struct cyttsp5_attribute *attr, char *buf)
{
	...
	rc = _cyttsp5_set_parameter_cmd(dev, 0x23, dad->opens_idac_mutual);
	rc |= _cyttsp5_get_parameter_cmd(dev, 0x23, &dad->opens_idac_mutual);
	rc |= _cyttsp5_set_parameter_cmd(dev, 0x24, dad->opens_atten_mutual);
	rc |= _cyttsp5_get_parameter_cmd(dev, 0x24, &dad->opens_atten_mutual);
	if (rc < 0) {
		dev_err(dev, "%s: Error on set parameters r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
	pr_info("%s: idac = %d, atten = %d\n",__func__, dad->opens_idac_mutual, dad->opens_atten_mutual);

	rc = cyttsp5_run_selftest_cmd_(dev, CY_ST_ID_CUSTOM_OPENS, 0,
			&cmd_status, &summary_result, NULL);
	if (rc < 0) {
		dev_err(dev, "%s: Error on run self test r=%d\n",
				__func__, rc);
	...
	rc = cyttsp5_get_selftest_result_cmd_(dev, 0, PIP_CMD_MAX_LENGTH,
			CY_ST_ID_CUSTOM_OPENS, &cmd_status,
			&act_length, &dad->ic_buf[10]);
	if (rc < 0) {
		dev_err(dev, "%s: Error on get self test result r=%d\n",
				__func__, rc);
		goto resume_scan;
	...
	cd = cyttsp5_get_core_data((char *)CY_DEFAULT_CORE_ID);
	/* Form response buffer */
	memcpy(dad->ic_buf, cd->response_buf, 10);

	length = 10 + act_length;
	/* set length to response buffer */
	dad->ic_buf[0] = (length & 0x00FF);
	dad->ic_buf[1] = (length & 0xFF00) >> 8;
	/* set actual length to response buffer */
	dad->ic_buf[7] = (act_length & 0x00FF);
	dad->ic_buf[8] = (act_length & 0xFF00) >> 8;
	/* set status to response buffer */
...
static ssize_t cyttsp5_opens_store(struct device *dev,
		struct cyttsp5_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_device_access_data *dad
		= cyttsp5_get_device_access_data(dev);
	...
	sscanf(buf,"%d %d",&idac,&atten);
	dad->opens_idac_mutual = idac;
	dad->opens_atten_mutual = atten;
...
static CY_ATTR(opens, S_IRUSR | S_IWUSR,
	cyttsp5_opens_show, cyttsp5_opens_store);
		...
		struct cyttsp5_attribute *attr, char *buf)
{
	...
	dad->calibrate_initialize_baselines = 1;
		...
		struct cyttsp5_attribute *attr, char *buf)
{
	...
	rc = cyttsp5_ret_scan_data_cmd_(dev, 0, 0x011B, dad->panel_scan_data_id,
			dad->ic_buf, &config, &actual_read_len, NULL);
	if (rc < 0) {
		dev_err(dev, "%s: Error Mutual scan1 r=%d\n", __func__, rc);
		goto resume_scan;
	}
	length = get_unaligned_le16(&dad->ic_buf[0]);
	buf_offset = dad->ic_buf + length;
	...
	rc = cyttsp5_ret_scan_data_cmd_(dev, elem_offset, 0x00B1, dad->panel_scan_data_id,
			NULL, &config, &actual_read_len, buf_offset);
	if (rc < 0) {
		dev_err(dev, "%s: Error Mutual scan2 r=%d\n", __func__, rc);
		goto resume_scan;
	}
	length += actual_read_len * element_size;
	buf_offset = dad->ic_buf + length;
	...
	rc = cyttsp5_ret_scan_data_cmd_(dev, elem_offset, 0x0037, dad->panel_scan_data_id,
			NULL, &config, &actual_read_len, buf_offset);
	if (rc < 0) {
		dev_err(dev, "%s: Error Mutual scan3 r=%d\n", __func__, rc);
		goto resume_scan;
	}
	length += actual_read_len * element_size;
	buf_offset = dad->ic_buf + length;
	...
	rc = cyttsp5_ret_scan_data_cmd_(dev, 0, 0x0024, dad->panel_scan_data_id,
			NULL, &config, &actual_read_len, buf_offset);
	if (rc < 0) {
...
static CY_ATTR(get_mu_se_diff, S_IRUSR, cyttsp5_mutual_self_diff_show, NULL);
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	rc = cyttsp5_create_file(dev, &cy_attr_get_mu_se_diff);
	if (rc) {
...
unregister_get_mu_se_diff:
	cyttsp5_remove_file(dev, &cy_attr_get_mu_se_diff);
#endif
unregister_baseline:
...
static int cyttsp5_device_access_probe(struct device *dev)
{
	...
	dad->opens_idac_mutual = DEFAULT_OPENS_IDAC_MUTUAL;
	dad->opens_atten_mutual = DEFAULT_OPENS_ATTEN_MUTUAL;
